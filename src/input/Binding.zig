...
pub const Set = struct {
    ...
    /// Exchange the horizontal arrow bindings that emit text, for a mirrored
    /// (RTL) display.
    ///
    /// Ghostty binds the modified arrows to fixed sequences on macOS -
    /// `alt+left` sends `ESC b` (word back), `super+left` sends `\x01` (line
    /// start), and so on. Those are resolved as keybindings, before key
    /// encoding, so the arrow swap applied there never sees them and they keep
    /// moving the cursor opposite to the arrow in a right-to-left row.
    ///
    /// Only pairs where BOTH sides emit text are exchanged. That deliberately
    /// leaves bindings like `super+alt+arrow` alone: those move pane focus,
    /// which follows the physical screen and must not mirror with the text.
    pub fn swapHorizontalTextArrows(self: *Set, alloc: Allocator) Allocator.Error!void {
        const emitsText = struct {
        ...
        // Collect first: mutating while iterating the map is not safe.
        var pairs: std.ArrayList(struct { mods: key.Mods, left: Action, right: Action }) = .empty;
        ...
        while (it.next()) |entry| {
            ...
            switch (trigger.key) {
                .physical => |k| if (k != .arrow_left) continue,
                else => continue,
            }
            const left_leaf = switch (entry.value_ptr.*) {
                .leaf => |l| l,
                // Leaders and multi-action bindings are left alone: they are
                // not a single text emission to mirror.
                .leader, .leaf_chained => continue,
            };
            if (!emitsText(left_leaf.action)) continue;

            const right_trigger: Trigger = .{
                .mods = trigger.mods,
                .key = .{ .physical = .arrow_right },
            };
            const right_entry = self.get(right_trigger) orelse continue;
            const right_leaf = switch (right_entry.value_ptr.*) {
                .leaf => |l| l,
                .leader, .leaf_chained => continue,
            };
            if (!emitsText(right_leaf.action)) continue;

            try pairs.append(alloc, .{
                .mods = trigger.mods,
                .left = left_leaf.action,
                .right = right_leaf.action,
            });
        }
...
test "swapHorizontalTextArrows: exchanges text arrows, leaves actions alone" {
    const testing = std.testing;
    const alloc = testing.allocator;
    var set: Set = .{};
    defer set.deinit(alloc);

    // The macOS defaults this exists to fix.
    try set.put(alloc, .{ .mods = .{ .alt = true }, .key = .{ .physical = .arrow_left } }, .{ .esc = "b" });
    try set.put(alloc, .{ .mods = .{ .alt = true }, .key = .{ .physical = .arrow_right } }, .{ .esc = "f" });
    try set.put(alloc, .{ .mods = .{ .super = true }, .key = .{ .physical = .arrow_left } }, .{ .text = "\x01" });
    try set.put(alloc, .{ .mods = .{ .super = true }, .key = .{ .physical = .arrow_right } }, .{ .text = "\x05" });
    // Pane focus follows the physical screen and must NOT mirror.
    try set.put(alloc, .{ .mods = .{ .super = true, .alt = true }, .key = .{ .physical = .arrow_left } }, .{ .goto_split = .left });
    try set.put(alloc, .{ .mods = .{ .super = true, .alt = true }, .key = .{ .physical = .arrow_right } }, .{ .goto_split = .right });

    try set.swapHorizontalTextArrows(alloc);

    const altLeft = set.get(.{ .mods = .{ .alt = true }, .key = .{ .physical = .arrow_left } }).?;
    const altRight = set.get(.{ .mods = .{ .alt = true }, .key = .{ .physical = .arrow_right } }).?;
    // alt+left now advances a word, which in a right-to-left row draws leftwards.
    try testing.expectEqualStrings("f", altLeft.value_ptr.leaf.action.esc);
    try testing.expectEqualStrings("b", altRight.value_ptr.leaf.action.esc);

    const superLeft = set.get(.{ .mods = .{ .super = true }, .key = .{ .physical = .arrow_left } }).?;
    const superRight = set.get(.{ .mods = .{ .super = true }, .key = .{ .physical = .arrow_right } }).?;
    // cmd+left reaches the visual left edge, which is the logical end of an RTL line.
    try testing.expectEqualStrings("\x05", superLeft.value_ptr.leaf.action.text);
    try testing.expectEqualStrings("\x01", superRight.value_ptr.leaf.action.text);

    const splitLeft = set.get(.{ .mods = .{ .super = true, .alt = true }, .key = .{ .physical = .arrow_left } }).?;
    const splitRight = set.get(.{ .mods = .{ .super = true, .alt = true }, .key = .{ .physical = .arrow_right } }).?;
    try testing.expectEqual(Action.SplitFocusDirection.left, splitLeft.value_ptr.leaf.action.goto_split);
    try testing.expectEqual(Action.SplitFocusDirection.right, splitRight.value_ptr.leaf.action.goto_split);
}

test "swapHorizontalTextArrows: ignores an unpaired arrow binding" {
    const testing = std.testing;
    const alloc = testing.allocator;
    var set: Set = .{};
    defer set.deinit(alloc);

    // Only one side bound: leave it where it is rather than moving it and
    // silently losing the binding the user pressed.
    try set.put(alloc, .{ .mods = .{ .ctrl = true }, .key = .{ .physical = .arrow_left } }, .{ .esc = "b" });
    try set.swapHorizontalTextArrows(alloc);

    const left = set.get(.{ .mods = .{ .ctrl = true }, .key = .{ .physical = .arrow_left } }).?;
    try testing.expectEqualStrings("b", left.value_ptr.leaf.action.esc);
    try testing.expect(set.get(.{ .mods = .{ .ctrl = true }, .key = .{ .physical = .arrow_right } }) == null);
}
