//! Bidi_Mirroring_Glyph (UAX #9 rule L4).
//!
//! When a character with Bidi_Mirrored=Yes is resolved to an odd (RTL)
//! embedding level, it is rendered as its mirrored counterpart: "(" becomes
//! ")", and the prompt ornament U+276F becomes U+276E.
//!
//! uucode exposes `is_bidi_mirrored` but not the mapping itself, so the pairs
//! are generated from the Unicode Character Database's BidiMirroring.txt.
//! Generated - do not edit by hand.

const std = @import("std");

/// Returns the mirrored form of `cp`, or null when the codepoint has no
/// Bidi_Mirroring_Glyph mapping.
...
test "mirror: known pairs round-trip" {
    const testing = std.testing;
    // The prompt ornament Claude Code draws.
    try testing.expectEqual(@as(?u21, 0x276E), mirror(0x276F));
    try testing.expectEqual(@as(?u21, 0x276F), mirror(0x276E));
    // Brackets.
    try testing.expectEqual(@as(?u21, ')'), mirror('('));
    try testing.expectEqual(@as(?u21, '('), mirror(')'));
    try testing.expectEqual(@as(?u21, ']'), mirror('['));
    try testing.expectEqual(@as(?u21, '>'), mirror('<'));
    // Letters and digits are never mirrored.
    try testing.expectEqual(@as(?u21, null), mirror('a'));
    try testing.expectEqual(@as(?u21, null), mirror(0x05D0));
}

test "mirror: every mapping is symmetric" {
    // L4 pairs are involutions: mirroring twice returns the original.
    var cp: u21 = 0;
    while (cp < 0x11000) : (cp += 1) {
        if (mirror(cp)) |m| {
            try std.testing.expectEqual(@as(?u21, cp), mirror(m));
        }
    }
}
